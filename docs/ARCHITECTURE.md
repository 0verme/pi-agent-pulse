...
`@earendil-works/pi-coding-agent` 是 host-provided peer dependency，开发基线固定为 `0.84.4`，已对照 `0.85.1` 的相关 lifecycle types 验证，保守兼容范围为 `>=0.84.4 <0.86.0`。`npm pack` 的发布内容由 package smoke 检查；发布时应使用同一份 deterministic `dist/` artifact。
