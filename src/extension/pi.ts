...
import type { OutboundChannel } from "../channels/types.js";
...
function boundedSummary(value: string | undefined): string | undefined {
	...
	const normalized = value
		.split(/\r?\n/)
		.map((line) =>
			/\b(?:token|password|secret|credential|authorization|webhook)\b|环境变量|令牌|密码|密钥|凭据|授权/i.test(
				line,
			)
				? "[redacted]"
				: line,
...
function repositoryName(cwd: string): string | undefined {
...
async function readGitBranch(pi: ExtensionAPI, cwd: string): Promise<string | undefined> {
...
function safeObserve(label: string, operation: () => void): void {
	try {
		operation();
	} catch {
...
export default function createPiPulseExtension(pi: ExtensionAPI): void {
	...
	pi.on("agent_start", (_event, ctx) => {
		safeObserve("agent_start", () => {
			...
			void readGitBranch(pi, ctx.cwd).then(
				(branch) => {
					if (branch) manager.updateTaskIdentity(sessionId, { branch });
				},
				() => undefined,
			);
		});
	...
	pi.on("turn_end", (event, ctx) => {
		...
		if (text.trim()) latestOutput = boundedSummary(text);
		touch(ctx);
	});
