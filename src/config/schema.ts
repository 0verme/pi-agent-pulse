...
function readString(parent: Record<string, unknown>, key: string, fallback: string): string {
	return typeof parent[key] === "string" ? parent[key].trim() : fallback;
}

function readTimeout(parent: Record<string, unknown>, key: string, fallback: number): number {
	const value = parent[key];
	if (typeof value !== "number" || !Number.isFinite(value)) return fallback;
	return Math.min(120_000, Math.max(100, Math.round(value)));
}

function readMinutes(parent: Record<string, unknown>, key: string, fallback: number): number {
	const value = parent[key];
	if (typeof value !== "number" || !Number.isFinite(value)) return fallback;
...
export function normalizeConfig(value: unknown): PulseConfig {
	...
	return {
		...
		watchdog: {
			longTaskNoticeMinutes: readMinutes(
				watchdog,
				"longTaskNoticeMinutes",
				DEFAULT_CONFIG.watchdog.longTaskNoticeMinutes,
			),
			longTaskWarningMinutes: readMinutes(
				watchdog,
				"longTaskWarningMinutes",
				DEFAULT_CONFIG.watchdog.longTaskWarningMinutes,
			),
			longTaskCriticalMinutes: readMinutes(
				watchdog,
				"longTaskCriticalMinutes",
				DEFAULT_CONFIG.watchdog.longTaskCriticalMinutes,
			),
...
export function loadConfigFile(filePath: string): PulseConfig {
	try {
